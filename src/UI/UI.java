...
//        System.out.println("regPassword:"+password);
        ...
        User user = new User(getAvailableMusicId(),username,password);
    ...
    static void logout(){
        try{
            FileWriter fi=new FileWriter("C:\\Users\\Administrator\\Desktop\\MusicPlayer-toko\\logged in.txt");
    ...
    static User getlogin() {
        ...
        String pass = "";
        try {
            FileReader fi = new FileReader("C:\\Users\\Administrator\\Desktop\\MusicPlayer-toko\\logged in.txt");
        ...
        Boolean l = false;
        for (int i = 0; i < userList.size(); i++) {
            if (Objects.equals(userList.get(i).getId(), Integer.parseInt(id)) && Objects.equals(userList.get(i).getPassword().hashCode(), Integer.parseInt(pass) + "")) {
    ...
    static void Upload(String input){
        ...
        String music=input.substring(3,input.length());
        MusicDao b=new MusicDao();
        List<Music> c=b.getAll();
        SQL.addMusic(new Music(getAvailableMusicId(),input.substring(3,input.length())));
    }
    ...
    static int getAvailableUserId(){//think it works pretty well :)) very nice horosho who is not ready to fight dandrum hah pow the burning you feel it is shape i have done more pain
        List<User> userList = userDao.getAll();
        int indx = 1;
        for(int i = 0; i < userList.size(); i++){
